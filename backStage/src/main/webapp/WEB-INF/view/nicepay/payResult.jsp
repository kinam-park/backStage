...
payHistVO.setResult_code(resultCode);
payHistVO.setResult_msg(resultMsg);
payHistVO.setAuth_date(authDate);
payHistVO.setAuth_code(authCode);
payHistVO.setBuyer_name(buyerName);
payHistVO.setMall_user_id(mallUserID);
payHistVO.setPay_method(payMethod);
payHistVO.setMid(mid);
payHistVO.setTid(tid);
payHistVO.setMoid(moid);
payHistVO.setAmt(amt);
payHistVO.setGoods_name(goodsName);
payHistVO.setCard_code(cardCode);
payHistVO.setCard_name(cardName);
payHistVO.setCard_quota(cardQuota);
payHistVO.setBank_code(bankCode);
payHistVO.setBank_name(bankName);
payHistVO.setRcpt_type(rcptType);
payHistVO.setRcpt_auth_code(rcptAuthCode);
payHistVO.setRcpt_tid(rcptTID);
payHistVO.setCarrier(carrier);
payHistVO.setDst_addr(dstAddr);
payHistVO.setVbank_bank_code(vbankBankCode);
payHistVO.setVbank_bank_name(vbankBankName);
payHistVO.setVbank_num(vbankNum);
payHistVO.setVbank_exp_date(vbankExpDate);

PaymentVO paymentVO = new PaymentVO();
paymentVO = (PaymentVO)session.getAttribute("paymentInfo");
...
if(paySuccess == true){
	...
	paymentService.insertPaymentProcess(paymentVO,payHistVO);
